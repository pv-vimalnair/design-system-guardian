...
Duplicated Figma working files are supported only for an exact `INSTANCE` whose current remote main-component key resolves to the approved published asset. During refresh, explicitly authorize the working file in the selected local profile, pin its version, and require catalog-authority-signed `workingFileInstances` evidence for its exact locator, selected variant/properties, and empty unapproved-override set. Include the exact `figmaInstance` locator in every affected component or icon resolution. A detached instance, cloned component definition, local component, changed override, visual match, or name match is `invalid`, never `missing`; stop instead of substituting. While the pinned snapshot contains any working-file binding, include an exact bound locator for every component and icon resolution; use a canonical-only snapshot for tasks that do not operate on a duplicate.
...
12. After every finalization outcome, run `guardian self-check --profile <profile-id> --run-id <run-id>`. Read only the authority-sealed post-run assessment. Report what worked, what failed or remained unassessed, and every exact reason code with its attribution and count. State whether review is recommended, that the handoff is `permission_required`, and that `sourceMutationPerformed` is false. Ask for the user's explicit permission before any Guardian evolution. Only after that permission may you use Plugin Evolution Manager or an equivalent portable evolution workflow; separate installation, publication, or other external-write authority is still required.

The build is complete only when finalization exits 0 and the sealed self-check has been reported. Exit 1 means a violation or sentinel remains; exit 2 means policy/configuration/integrity failure; exit 3 means unavailable, stale, or incomplete source; exit 4 means unsupported adapter or incomplete/not-assessed coverage. Version 0.3.1 has no trusted UX/accessibility evaluator, so that lane is always canonicalized to `not_assessed` and the private pilot cannot exit 0.
