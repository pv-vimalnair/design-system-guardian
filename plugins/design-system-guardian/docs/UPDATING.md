...
This section records the intended future production flow. Version 0.3.0 can prepare and externally sign evidence, but its public channel operation is an unconditional blocker and cannot complete step 8.
...
8. After a future reviewed release integrates the fixed provider, supply the signed manifest and exact artifact to `guardian_core.release.promote_release`; that implementation must authenticate latest head and complete monotonic CAS. In version 0.3.0 this call always blocks and accepts no configuration workaround.
...
The following procedure is also reserved for a future reviewed release with the fixed provider integrated; version 0.3.0 always blocks the public rollback call.
